...
const GridRowSpan = [GridCellSize, GridCellSize, GridCellSize]
const GridColumnSpan = [GridCellSize, GridCellSize]

const Select = styled.div`
  display: grid;
  grid-template-rows: ${GridRowSpan.join('px ')};
  grid-template-columns: ${GridColumnSpan.join('px ')};
  align-items: stretch;
  justify-items: stretch;
  background: ${BackgroundColor.base};
  box-shadow: ${SelectShadow};
  width: ${GridCellSize * GridColumnSpan.length}px;
  height: ${GridCellSize * GridRowSpan.length}px;
`
...
const PositionOptions = ({ position, span, spanHover }) => {
  ...
  const gridRows = Object.keys(GridRowSpan)
  const gridColumns = Object.keys(GridColumnSpan)

  return gridRows.reduce(
    (positions, _, row) =>
      positions.concat(gridColumns.map((_, column) => getPosition(row + 1, column + 1))),
    []
  )
}
...
export const SelectAreaMobile = ({ position, span, spanHover, onChange, onSelecting }) => {
